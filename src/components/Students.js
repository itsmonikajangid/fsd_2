...
let updatedStudentList = studentList.filter((e) => e.id !=
id);
setStudentList(updatedStudentList);
updatedStudentList = filteredStudentList.filter((e) =>
e.id != id);
...
{
if(e.id==student.id)
{ // return modified student object
return student;
}
else
{
...
let [filteredStudentList, setFilteredStudentList] =
useState([]);
...
content_header = "Total " + filteredStudentList.length + "
students";
}
return (
<div className="container container-students p-2">
{message && <div className="div-message">{message}</div>}
{view == "list" && (
<div className="div-student-count">{content_header}</div>
)}
{view == "list" && <Search
onSearchTextKeyUp={handleSearchTextKeyUp} />}
<div className="mb-2">
<button className="btn btn-light"
onClick={handleAddOrListButtonClick}>
{view == "list" ? "Add a Student" : "Show Student Records"}
</button>
</div>
{view == "list" && filteredStudentList.length != 0 && (
<div>
<div className="row">
<div className="col-1">S.N.</div>
<div className="col-2">Roll No.</div>
